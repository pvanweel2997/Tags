...
export class ArticleService {
  constructor(
    ...
    private readonly folllowRepository: Repository<FollowEntity>,
  ...
  async findAll(
    ...
    if (query.author) {
      ...
      queryBuilder.andWhere('articles.authorId = :author', {
        author: id,
      });
    }
  ...
  async deleteArticleToFavorites(
    ...
    if (articleIndex >= 0) {
      delete user.favorites[articleIndex];
      article.favoritescount--;
      await this.userRepository.save(user);
      await this.articleRepository.save(article);
    }
  ...
  async getFeed(
    ...
    const follows = await this.folllowRepository.find({
      where: {
